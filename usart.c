...
void USART_Init_Config(USART_Handle_t* usart1_handle)
{
	...
	else if(usart1_handle->usart_port==USART2)
	{
		RCC->APB1ENR|=RCC_APB1ENR_USART2EN;
	}
	else if(usart1_handle->usart_port==USART3)
	{
...
void TaskSendUsart(void *pvParameters)
{
	(void)*pvParameters;
	USART1_Config();
	...
	while(1)
	{
		snprintf(tx_buffer,sizeof(tx_buffer),"HELLO FROM STM32!\r\n");
		USART_Send(&usart_handle,tx_buffer);
